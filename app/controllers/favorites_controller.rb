class FavoritesController < ApplicationController
  ...
  def index
    ...
    @favorites = Favorite.where(user_id: @user.id).order("RANDOM()").page(params[:page]).per(20)
  end

  def show_favorites
    @artist = Artist.find(params[:id])
    @count = Favorite.where(artist_id: @artist.id).all
    @favorites = Favorite.where(artist_id: @artist.id).all.order("RANDOM()").page(params[:page]).per(20)
  end
